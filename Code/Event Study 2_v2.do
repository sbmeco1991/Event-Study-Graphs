...
gen mml_lead1 = 0                /* This is the base/reference category */
...
*Storing the coeffcients

estimates store leads_lags


/***************************************************/
*Trying to plot the event study graph using coefplot*
/***************************************************/

#delimit ;

coefplot leads_lags, keep(mml_lead5 mml_lead4 mml_lead3 mml_lead2 mml_lead1 mml mml_lag1 mml_lag2 mml_lag3 mml_lag4 mml_lag5)
                     baselevels omitted
                     vertical title("{stSerif:{bf:Figure 2. {it:Trends in Traffic Fatalities}}}", color(black) size(large)) 
                     xtitle("{stSerif:Years Since Medical Marijuana was Legalized}") xscale(titlegap(2)) xline(6, lcolor(black)) 
...
note("{stSerif:{it:Notes}. OLS coefficient estimates (and their 95% confidence intervals) are reported.}" 
     "{stSerif:The dependent variable is log of traffic fatalities per 100,000 population in state s {it:s} and} year {it:t}." 
...
ilwidth(vvvthin)) ciopts(lwidth(*3) lcolor(black)) mcolor(black) scheme(plotplain);

#delimit cr

graph export "$path\Graphs\Traffic_Fatalities_ES_v2.png", as(png) replace
